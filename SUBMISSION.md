...
- **Uniswap AI Skills used** (see `agent/src/adapters/UniswapSkillsAdapter.ts`)
  - `liquidity-planner@0.2.0` — ported verbatim to TypeScript so the backend monitoring loop can use it without spawning a Claude Code sub-agent every 5 minutes. The adapter exports `LIQUIDITY_PLANNER_SKILL = { name, version, source, installedPath }` as a runtime citation, and every `PoolAnalysis.reasoning` string closes with `"Range math via liquidity-planner@0.2.0 (<github url>)."`. Ported constants (all 1:1 from the skill):
  ...
  - The ranges the skill computes drive `AgentCoordinator.rebalanceViaOnchainOS` as **directional trigger bands**: cross-above the upper bound → sell non-stable, cross-below the lower bound → buy non-stable. The same width table and fee-tier map that would drive a V3 NFT mint decision now drives a swap direction — the Uniswap skill stays load-bearing even though the execution surface changed.
...
3. **`collectViaOnchainOS` is a no-op stub.** In swap mode we hold no V3 NFT, so there are no fees to `defi collect`. The stub returns `null` by design; `AgentCoordinator.runCompound()` detects the null return and writes a `logHold` heartbeat instead of a `compoundFees` row. This keeps the on-chain audit trail honest: every `ACTION_COMPOUND` row in `StrategyManager.getExecutions(strategyId)` corresponds to a real harvest tx hash on OnchainOS. The earlier (pre-2026-04-11) version of `runCompound` had a bug where it wrote a `compoundFees` row unconditionally on every 6-hour tick; that bug was self-caught during code review, fixed in the same commit as this Known Limitations section, and the monitor loop was stopped before the audit trail accumulated more than a handful of now-explained rows.

4. **OnchainOS CLI account drift.** Running `onchainos wallet login --force` from a stale session occasionally rebinds the CLI to a different account (our setup has a funded Account 1 and an empty Account 2). The agent's `OnchainOSAdapter.ensureLogin()` handles the common case, but if judges ever see a "zero balance" error on the `/api/deploy` smoke check, the fix is `onchainos wallet switch 04c9d299-9e85-4c20-98c5-8f1f2a4bba36` to force Account 1 (funded, TEE signer `0x6ab27b82…`).
